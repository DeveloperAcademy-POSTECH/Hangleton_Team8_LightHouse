...
struct Constants {
    ...
    static let GestureArray = [
        Gesture(mainCategory:.requiredFeatures,title:"VoiceOver를 시작하기",subTitle:"VoiceOver를 켜거나 끄는 방법",content:"총 세 가지 방법이 있습니다.\n\n첫 번째로, 시리를 호출한 후 “보이스오버 켜줘”라고 말할 시 VoiceOver 기능을 사용하실 수 있습니다. 다시 끄고 싶은 경우에는 시리를 호출한 후 “보이스오버 꺼줘”라고 말해보세요. \n\n두 번째로, 설정 앱에서 “손쉬운 사용”으로 이동 후 VoiceOver 화면으로 이동하면, VoiceOver를 켜고 끌 수 있습니다. \n\n마지막으로, 설정 앱에서 “손쉬운 사용”으로 이동 후 “손쉬운 사용 단축키”에서 VoiceOver를 단축키로 설정할 수 있습니다. 설정 후에 홈버튼이 없는 기종의 경우 전원 버튼을 삼중 클릭하면 VoiceOver를 끄고 킬 수 있습니다. 홈버튼이 있는 기종의 경우에는 홈 버튼을 삼중 클릭하세요.",imageName:"startVoiceOver"),
        Gesture(mainCategory:.requiredFeatures,title:"세 손가락으로 삼중 탭",subTitle:"화면 커튼 토글",content:"화면 커튼이 켜져 있으면 화면을 완전히 어둡게 만들어줍니다. 다른 사람에게는 화면이 꺼져 보이지만, 기기 및 VoiceOver 탐색이 활성 상태로 유지됩니다. 개인정보 보호 기능을 강화 시 키기기 위해서 해당 기능을 사용하실 수 있습니다. \n\n한 번 더 세 손가락으로 삼중 탭 할 경우 화면 커튼 기능이 꺼집니다. \n\n손쉬운 사용의 확대/축소가 활성화되어 있으면 세 손가락으로 사중 탭 하기 제스처를 통해 해당 기능을 이용할 수 있습니다.",imageName:"threeFingerTripleTap"),
        Gesture(mainCategory:.requiredFeatures,title:"두 손가락으로 문지르기",subTitle:"Escape",content:"이전 화면으로 이동하거나 화면을 닫습니다. 두 손가락을 Z모양으로 드래그하면 해당 화면의 이전 화면으로 이동하거나 제어센터, 알림센터, 앱 전환기, 앱폴더 등 닫기 버튼이 따로 없는 화면을 닫습니다. 키보드가 열려 있을 경우에는 키보드를 접을 수 있습니다.",imageName:"twoFingerScrub"),
        Gesture(mainCategory:.requiredFeatures,title:"로터 기능 알아보기",subTitle:"로터 사용법",content:"로터를 사용하여 음량, 말하기 속도, 음높이 변경 또는 발음 기호 사용, 에코 입력 및 구두점 읽기와 같은 설정을 조절할 수 있습니다.\n\n화면에 두 손가락을 올려놓은 채로 두 손가락을 시계 또는 반시계 방향으로 회전시켜서 사용이 가능합니다. 양손을 사용하는 경우에는 한 손가락은 고정해두고 나머지 한 손가락을 시계 또는 반시계 방향으로 드래그하세요.",imageName:"rotorClounterClockwise"),
        Gesture(mainCategory:.convenienceFeatures,title:"홈으로 이동하기",subTitle:"홈으로 이동하기 위한 방법",content:"홈버튼이 없는 기종일 경우 알림음이 2번 울릴 때까지 화면 하단 가장자리에서 위의 방향으로 쓸어 올리세요. \n\n홈버튼이 있는 기종의 경우에는 홈 버튼을 한번 클릭하세요.",imageName:"moveToHome"),
        Gesture(mainCategory:.convenienceFeatures,title:"앱 전환기",subTitle:"앱 전환기를 실행하기 위한 방법",content:"홈버튼이 없는 기종일 경우 알림음이 3번 울릴 때까지 화면 하단 가장자리에서 위의 방향으로 길게 쓸어 올리세요. \n\n홈버튼이 있는 기종일 경우 홈 버튼을 두 번 클릭하세요. \n\n앱 전환기에 있는 앱을 삭제하고 싶은 경우 해당 앱을 선택하고 세 손가락을 위로 드래그하세요.",imageName:"appSwitcher"),
        Gesture(mainCategory:.convenienceFeatures,title:"제어센터",subTitle:"제어센터를 열거나 닫기 위한 방법",content:"시간, 배터리 등 상태 아이콘이 표시되어 있는 최상단의 상태 막대를 탭한 후 세 손가락을 위쪽 방향으로 쓸어 올리세요. \n\n제어센터를 닫으려면 두 손가락으로 문지르기를 하시면 됩니다.",imageName:"controlCenter"),
        Gesture(mainCategory:.convenienceFeatures,title:"알림센터",subTitle:"알림센터를 열거나 닫기 위한 방법",content:"시간 및 베터리가 표시되어 있는 최상단의 상태 막대를 탭한 후 세 손가락을 아래쪽 방향으로 쓸어 내리세요. \n\n알림센터를 닫으려면 두 손가락으로 문지르기를 하시면 됩니다.",imageName:"notificationCenter"),
